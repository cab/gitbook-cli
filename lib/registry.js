  ...
  return initNPM()
    ...
    .then(function(result) {
      ...
      if (result.versions.length == 0)
        throw new Error("No valid version on the NPM registry");
      return result;
    });
}
...
function resolveVersion(version) {
  ...
  return availableVersions().then(function(available) {
    // Resolve if tag
    ...
    if (!version)
      throw new Error(
        'Invalid version or tag "' +
          _version +
          '", see available using "gitbook ls-remote"'
      );
    return version;
  });
}
  ...
  return resolveVersion(version)
    ...
    .then(function(tmpDir) {
      ...
      var packageJson = fs.readJsonSync(
        path.resolve(gitbookRoot, "package.json")
      );
      ...
      if (!tags.isValid(version))
        throw "Invalid GitBook version, should satisfies " +
          config.GITBOOK_VERSION;

      // Copy to the install folder
      return Q.nfcall(fs.copy.bind(fs), gitbookRoot, outputFolder).thenResolve(
        version
      );
    });
}
